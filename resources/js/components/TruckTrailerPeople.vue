        ...
        <div style="padding:20px;">
            ...
            <div class="row">
                <div class="input-group mb-1 col">
                    <div class="col-md-8">
                        <select v-model="selectedClient" class="custom-select" id="clientSelector">
                            <option selected>Choose Client</option>
                            <option v-for="client in truckTrailerDriver.clients" :key="client.id" :value="client.id" aria-placeholder="Choose Client">{{client.client_name}}</option>
                        </select>
                    </div>
                </div>
                <div class="input-group mb-1 col">
                    <div class="col-md-8">
                        <select class="custom-select" id="cargoSelector">
                            ...
                            <option v-for="cargo in truckTrailerDriver.cargo" :key="cargo.id" :value="cargo.id">{{cargo.cargo_name}}</option>
                        </select>
                    </div>
                </div>
                <div class="input-group mb-1 col">
                    <div class="col-md-8">
                        <select class="custom-select" id="destinationSelector">
                            ...
                            <option v-for="location in truckTrailerDriver.locations" :key="location.id" :value="location.id">{{$location.location_name}}</option>
                        </select>
                    </div>
                </div>
            </div>
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered table-sm table-striped table-hover" id="dataTable" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        <tr v-for="ttp in truckTrailerDriver.truck_trailer_people" :key="ttp.id">
                            <td><input class="form-check-input ml-2" type="checkbox" value="" :id="ttp.id"
                                    @click="trucksArray(ttp.id)"></td>
